...
fixture `TIME TO RESOLVE - GRAFANA - ONE DAY`
    .page(DesktopHelper.urlGrafana);
    
test('Otestování, že v grafaně - Dashboard(FourKeys), Median Time to Restore Services je ONE DAY ', async (t: TestController) => {
    await t
        ...
        .expect(Selector('main span').withText('One year').exists).ok()
});
